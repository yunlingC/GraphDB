...
public:
  ...
  virtual bool visitVertex(VertexPointer vp) {
    ...
    auto VId = graph.addVertex();
    auto VertexId = graph.getVertexPointer(VId);
    LockManager.addToVertexLockMap(VId);
...
//    cout << "check locks number " <<  LockManager.getVertexLockMap().size() << "\n";
    while( !Lock ) {
      Lock = LockManager.getEdgeLock(edge, Pp, SH);
    }

    ReturnValue = checkType(edge, _Filter);
    LockManager.releaseEdgeLock( edge, Pp, SH);
...
//    Graph.getVertexPointer(VertexId);
  ...
  virtual bool visitVertex(VertexPointer first) {
      if (first->getId() == VertexId )
        return false;
    auto SourceP = first;
    auto VertexP = Graph.getVertexPointer(VertexId);
    auto VertexIdLock = false;

    while ( !VertexIdLock ) {
      VertexIdLock = LockManager.getVertexLock(SourceP, ID, SH);
    }

    ReturnBranchType VertexMatch = checkBranch<ReturnValueType>(first, getFilter());
    if( !VertexMatch.first )  {
      LockManager.releaseVertexLock(SourceP, ID, SH);
      return false;
    }

    ///TODO should be moved to graphType.h

    auto VLELock = LockManager.getVertexLock(VertexP, LE, EX);
    auto VNELock = LockManager.getVertexLock(VertexP, NE, EX);
    auto SLELock = LockManager.getVertexLock(SourceP, LE, EX);
    auto SNELock = LockManager.getVertexLock(SourceP, NE, EX);

    while( !VLELock || !VNELock || !SLELock || !SNELock) {
      if ( VLELock ) 
        LockManager.releaseVertexLock(VertexP, LE, EX);
      if ( VNELock )
        LockManager.releaseVertexLock(VertexP, NE, EX);
      if ( SLELock ) 
        LockManager.releaseVertexLock(SourceP, LE, EX);
      if ( SNELock )
        LockManager.releaseVertexLock(SourceP, NE, EX);

      std::this_thread::yield();

      VLELock = LockManager.getVertexLock(VertexP, LE, EX);
      VNELock = LockManager.getVertexLock(VertexP, NE, EX);
      SLELock = LockManager.getVertexLock(SourceP, LE, EX);
      SNELock = LockManager.getVertexLock(SourceP, NE, EX);
    }
    ...
    if(VertexMatch.second.second.get("null").second != true) {
      NewEdgeId = Graph.addEdge(VertexId, SourceP->getId(),
...
/**
class AddEdgeVisitor : public ConcurrentVisitor {
public:
  typedef pair<FixedString, bool> ReturnValueType;
  typedef PropertyList<FixedString, FixedString> PropertyListType;
  typedef std::pair<bool, std::pair<std::string, GraphType::PropertyListType> > ReturnBranchType;
...
public:
  ...
  AddEdgeVisitor (LockManagerType & lm, GraphType & graph) :
                LockManager(lm), graph (graph) {}
